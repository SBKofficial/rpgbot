...
def get_logs(uid, pid):
    ...
    except Exception: return r"❌ Error reading logs\."
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        r"• `/upload [name]` — Save and push to GitHub\." + "\n"
        r"• `/sync` — Manual GitHub push\." + "\n\n"
        r"🛰 *Monitoring*" + "\n"
        r"• `/deployments` — Active task list\." + "\n"
        r"• `/logs [slug]` — Snapshot of logs\." + "\n\n"
        r"▶️ *Control*" + "\n"
        r"• `/run [slug] [cmd]` — Start process\." + "\n"
        r"• `/stop [slug]` — Kill process\." + "\n"
        r"• `/send [slug] [text]` — Input to stdin\."
    )
...
async def status_cmd(update, context):
    ...
    for item in sorted(os.listdir(base)):
        ...
        icon = "🐍" if item.endswith(".py") else "🟢" if item.endswith(".js") else "📄"
        kb.append([InlineKeyboardButton(f"{icon} {item}", callback_data=f"manage_{item}")])
